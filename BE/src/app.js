...
const ROUTE_METHODS = ["get", "post", "put", "patch", "delete", "options", "head"];
...
const GENERIC_PASSWORD_RESET_MESSAGE =
  "Nếu tên đăng nhập tồn tại, yêu cầu quên mật khẩu đã được ghi nhận và admin sẽ thấy cảnh báo để xử lý.";
...
function sanitizeUser(user) {
  ...
  return {
    ...
    enterprise:
      ...
      inferEnterpriseFromBranch(user.branch_name || "") ||
      null,
...
async function getBranchesTree() {
  ...
  return normalizeOrganizationsTree(
    Array.isArray(rows[0]?.data) ? rows[0].data : [],
  );
}
...
async function saveBranchesTree(branches) {
  await pool.query(
    ...
    [
      "branches_tree",
      JSON.stringify(normalizeOrganizationsTree(branches || [])),
    ],
  );
}
...
app.post("/api/auth/login", async (req, res) => {
  ...
  await pool.query("UPDATE users SET last_login_at = $1 WHERE id = $2", [
    now,
    user.id,
  ]);
...
app.post(
  ...
  async (req, res) => {
    ...
    const enterprise =
      role === "admin"
        ? null
        : normalizeEnterpriseName(req.body?.enterprise || "");
    const branch =
      role === "admin" ? null : normalizeBranchName(req.body?.branch || "");
    ...
    if (!/^[a-zA-Z0-9._-]{3,30}$/.test(username)) {
      return failure(
        res,
        "Tên đăng nhập chỉ nên gồm chữ, số, dấu chấm, gạch dưới hoặc gạch ngang (3-30 ký tự).",
      );
    }
    ...
    if (role !== "admin" && !branch) {
      return failure(
        res,
        "Phải chọn cung phụ trách cho tài khoản cung trưởng.",
      );
    }
    ...
    if (role !== "admin") {
      ...
      const branchResult = ensureOrganizationBranchStructure(
        fullBranches,
        enterprise,
        branch,
        stations,
    ...
    const password =
      String(req.body?.password || "").trim() || generateUniquePassword();
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!user)
      return failure(
        res,
        "Không tìm thấy tài khoản cần cấp lại mật khẩu.",
        404,
      );

    const password =
      String(req.body?.nextPassword || "").trim() || generateUniquePassword();
    const passwordHash = await bcrypt.hash(password, 10);
    const now = new Date().toISOString();
...
app.put(
  "/api/branches/tree",
  requireAuth,
  requireRole(["admin", "cungtruong"]),
  async (req, res) => {
    const incomingBranches = Array.isArray(req.body?.branches)
      ? req.body.branches
      : [];
    const fullBranches = await getBranchesTree();
    const nextBranches = mergeVisibleOrganizationsIntoFull(
      fullBranches,
      incomingBranches,
      req.user,
    );
    ...
    return success(res, {
      message: "Đã lưu dữ liệu cây xí nghiệp/cung/ga/tài sản.",
      data: filterOrganizationsForUser(nextBranches, req.user),
    });
  },
);
...
app.get(
  ...
  async (req, res) => {
    ...
    return success(res, {
      data: rows.map(normalizePasswordResetRequest),
    });
  },
);
...
app.post('/api/password-reset-requests', async (req, res) => {
  ...
  return success(res, {
    created,
    message: GENERIC_PASSWORD_RESET_MESSAGE,
    request: normalizePasswordResetRequest(requestRecord),
  }, created ? 201 : 200);
});
app.patch(
  "/api/password-reset-requests/:username/resolve",
  requireAuth,
  requireRole(["admin"]),
  async (req, res) => {
    const username = String(req.params?.username || "").trim();
    if (!username)
      return failure(res, "Thiếu tài khoản cần xử lý yêu cầu quên mật khẩu.");
    ...
    if (!rows[0]) {
      return failure(
        res,
        "Không còn yêu cầu quên mật khẩu đang chờ xử lý cho tài khoản này.",
        404,
      );
    }

return success(res, {
  request: normalizePasswordResetRequest(rows[0]),
});
  },
);
